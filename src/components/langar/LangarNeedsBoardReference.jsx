...
const LangarNeedsBoardReference = ({ items = [], triggerOnly = false, navItem = false, onOpen }) => {
  ...
  const submitContribution = (event) => {
    ...
    contributionMutation.mutate({ itemId: selectedItem.id, itemName: selectedItem.name, quantity: Number(quantity), unit: selectedItem.unit, donorName: donorName || user?.name || 'Member', donorEmail: user?.email || '', donorAvatarUrl: user?.avatarUrl || user?.picture || user?.photoURL || '', anonymous, expectedDeliveryDate: deliveryDate });
  };
  ...
  return <>
    {triggerOnly ? (
      navItem ? (
        <button type="button" onClick={openBoard} className="inline-flex flex-col items-center rounded-xl border border-amber-300 bg-white px-2 py-1.5 text-[10px] font-bold text-amber-700 hover:bg-amber-50">
          <ArchiveBoxIcon className="h-4.5 w-4.5" />
          Langar
        </button>
      ) : (
        <button type="button" onClick={openBoard} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800 hover:bg-amber-100"><ArchiveBoxIcon className="h-4 w-4" /> Help stock the kitchen</button>
      )
    ) : homeCard}
    {boardOpen ? <div className="fixed inset-0 z-[300] overflow-y-auto bg-slate-950/70 px-3 py-5 backdrop-blur-sm sm:px-5" onClick={() => setBoardOpen(false)}><div className="mx-auto max-w-6xl overflow-hidden rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="Help us stock the kitchen" onClick={(event) => event.stopPropagation()}>
